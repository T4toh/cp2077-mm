...
CREATE OR REPLACE MACRO loadouts.TrackedEntitiesForLoadout(db, loadoutId) AS TABLE
...
UNION ALL
SELECT linked.Id FROM MDB_LIBRARYLINKEDLOADOUTITEM(Db=>db) linked 
    JOIN mdb_LoadoutItem(Db=>db) item ON linked.Id = item.Id
    WHERE item.Loadout = loadoutId
UNION ALL
SELECT Id FROM MDB_COLLECTIONGROUP(Db=>db) WHERE Loadout = loadoutId
UNION ALL
SELECT Id FROM MDB_LOADOUTOVERRIDESGROUP(Db=>db) WHERE Loadout = loadoutId
UNION ALL
